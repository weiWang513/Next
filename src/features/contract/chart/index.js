...
function Chart() {
  ...
  const [Type, setType] = useState(
    Number(localStorage.getItem("KLINE_TYPE")) || 0
  ); //图表类型
  const [chartType, setChartType] = useState(
    Number(localStorage.getItem("KLINE_CHARTTYPE")) || 0
  ); //kline类型
  const [indictorMaster, setIndictorMaster] = useState(
    localStorage.getItem("KLINE_MAIN_INDICTOR") || ""
  ); //当前选中副指标
  const [indictorSub, setIndictorSub] = useState(
  ...
  const futureKline = useAppSelector((state) => state.contract.futureKline);
  const futureLastestTickPrice = useAppSelector(
    (state) => state.contract.futureLastestTickPrice
  );
  const resolution = useAppSelector((state) => state.contract.resolution);
  // const posListProps = useAppSelector((state) => state.contract.posListProps);
  ...
  useEffect(() => {
    dispatch(
      changeResolution(localStorage.getItem("KLINE_RESOLUTION") || "15")
    );
  ...
  const _indictor = (l, type) => {
    if (Type === 0) {
      if (l.type === 0) {
        l.label === indictorMaster
          ? setIndictorMaster("Close")
          : setIndictorMaster(l.label);
        localStorage.setItem(
          "KLINE_MAIN_INDICTOR",
          l.label === indictorMaster ? "Close" : l.label
        );
      } else {
        l.label === indictorSub
          ? setIndictorSub("Close")
          : setIndictorSub(l.label);
        localStorage.setItem(
          "KLINE_SUB_INDICTOR",
          l.label === indictorSub ? "Close" : l.label
        );
      }
    } else if (Type === 1) {
              ...
              {btnListFixed.map((l, index) => {
                return (
                  <span
                    key={index}
                    className={
                      l.chartType === chartType && l.resolution === resolution
                        ? "active "
                        : ""
                    }
                    onClick={() => _change(l)}
          ...
          <div className="tool-panel">
                    ...
                    {btnList.map((l, index) => {
                      return (
                        <PanelItem
                          key={index}
                          className={
                            l.chartType === chartType &&
                            l.resolution === resolution
                              ? "active"
                              : ""
                          }
                          mr={(index + 1) % 5 === 0 ? "0px" : "8px"}
                          onClick={() => {
                            _change(l);
            ...
            {/* {Type === 0 && (
              <Dropdown
                ...
                targetHoverStyle={IndictorIconHover}
              >
                <IndictorPanel>
                  <PanelTitle>主图指标:</PanelTitle>
                  <PanelItemWrap>
                  ...
                  <PanelTitle mt={"16px"}>副图指标:</PanelTitle>
                  <PanelItemWrap>
            ...
            {Type === 1 && (
              <img
                src={"/images/chart/indictor.png"}
                onClick={() => _indictor()}
              />
            )}
            {Type === 1 && (
              <img src={"/images/chart/camera.png"} onClick={() => _camera()} />
            )}
            {Type === 1 && (
              <img
                src={"/images/chart/setting.png"}
                onClick={() => _setting()}
              />
            )}
          </div>
  ...
  return !fullScreen ? (
    <ChartContainer
      className={
        fullScreen ? "chart-container full-chart-container" : "chart-container"
      }
    >
      {renderToolBar()}
      {renderChart()}
