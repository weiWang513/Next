...
const priceTick = () => {
  ...
  let contractId = store.getState().contract.contractId;
  let contractItem = store.getState().contract.contractItem;
  if (contractId && contractItem.priceTick) {
    length =
      scientificNotationToString(Number(contractItem.priceTick) || "0")?.split(".")[1]?.length || 2;
