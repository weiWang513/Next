...
import { queryFlOrders } from "../../../../services/api/contract";
import {
  dateFormat2,
  toFix6,
  formatByPriceTick,
  ...
  return (
    <Table>
      ...
      <div className="table-head">
        ...
        <span>
          {t("closeQuantity")}({t("Cont")})
        </span>
        <span>{t("closePrice")}</span>
        <span>{t("closeAmt")}</span>
            ...
            <div className="table-body">
              {list.map((el, index) => (
                <section className="table-row" key={index}>
                  ...
                  <span>{formatByPriceTick(el.closePrice, el.contractId)}</span>
                  <span>{toFix6(el.closeAmt)}</span>
                  <span>
                    {el.profitLossSide === -1
                      ? t("BankruptcyReduction")
                      : t("AutomaticLightening")}
                  </span>
                  <span>
