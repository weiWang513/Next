...
import { getServiceProvider, evaluate, quote, payment } from "../../../services/api/otc";
...
const BuyCoin = () => {
  ...
  const [otcList, setOtcList] = useState<object>([]);
  ...
  const filterCurrencyAndFait = (f, v, s = 0, serviceProviderList, isInit?) => {
    ...
    let _serviceProviderId = serviceProviderItem?.serviceProviderId;
    let _serviceProviderItem = _merchantsL.find(
      (el) => el.serviceProviderId === _serviceProviderId
    );
    setServiceProviderItem(_serviceProviderItem ? _serviceProviderItem : _merchantsL[0]);
  };
  ...
  const Enquiry = async () => {
    ...
    let _list = await Promise.all(
      Array.apply(null, { length }).map(async (item, index) => {
        item = {
          serviceProviderId: merchantsList[index]?.serviceProviderId
        };

        let _EnquiryItem = await EnquiryItem(item.serviceProviderId);
    ...
    let _during: any = _list.find(
      (el: any) => el.serviceProviderId === serviceProviderItem?.serviceProviderId
    );
  ...
  const EnquiryItem = async (serviceProviderId) => {
    let ratio = 0;
    let validUntil = 0;
    let quoteId = null;
    ...
    if (serviceProviderId === 0) {
      let _queryAllOtcCurrencyList = await queryAllOtcCurrencyList();

      _queryAllOtcCurrencyList.data.code === 0 && setOtcList(_queryAllOtcCurrencyList.data.data);
      ratio = _queryAllOtcCurrencyList.data.data[0].buyPrice;
      validUntil = 30;
    }
    ...
    if (serviceProviderId) {
      ...
      if (isEvaluate) {
        ...
        if (data.code === 0) {
          console.log("Enquiry evaluate", data.data);
        ...
        if (data.code === 0) {
          console.log("Enquiry quote", data.data);
  ...
  const handleChangeFaitCurrency = (item) => {
    console.log("handleChangeFaitCurrency", item);
  ...
  const init = () => {
    queryAllOtcCurrencyList().then((res) => {
      if (res.data.code === 0) {
        setOtcList(res.data.data);

        let _otc365 = res.data.data[0];
        ...
        getServiceProvider().then((res) => {
          ...
          if (data.code === 0) {
            ...
            let _l = [];
            let _id;
            let _serviceProviderList = [
              ...data.data.filter((el) => el.status === 1),
              {
                purchaseEnabled: true,
                saleEnabled: true,
                serviceProviderId: 0,
                serviceProviderName: "OTC365",
                status: 1,
                supportCurrencyInfo: [
                  {
                    currencyId: 7,
  ...
  const handleNumChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (usdtType) {
      //按数量购买
      if (serviceProviderItem?.serviceProviderId) {
        //非 otc365
        console.log("decimals", currencyIdItem?.decimals);
  ...
  const _setRatiolist = (value, _ratiolist) => {
    ...
    let ratioItem = _ratiolist?.find(
      (r) => r?.serviceProviderId === serviceProviderItem?.serviceProviderId
    );
    if (ratioItem) {
      if (usdtType) {
        let numTemp;
        if (serviceProviderItem?.serviceProviderId === 0) {
  ...
  const onBuy = () => {
    ...
    if (serviceProviderItem?.serviceProviderId) {
      let paymentParams = {
        ...
        params: {
          ...
          quoteId: ratiolist?.find(
            (el) => el.serviceProviderId === serviceProviderItem?.serviceProviderId
          )?.quoteId
        }
      };
      ...
      buy({
        params: {
          buyType: 1,
          currencyId: otcList[0].currencyId,
          quantity: getNum,
          syncUrl: `http://${window.location.host}`
        },
        headers: { unique: uuid() }
      }).then((res) => {
        if (res.data.code === 0) {
