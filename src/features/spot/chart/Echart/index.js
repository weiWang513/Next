...
import { formatBigNumber } from "../../../../utils/filters";
...
function Depth() {
  ...
  const contractItem = useAppSelector((state) => state.spot.currentSpot);
  const contractId = useAppSelector((state) => state.spot.spotId);
  const futureQuot = useAppSelector((state) => state.spot.snapshot);
  const bidAskData = useAppSelector((state) => state.spot.orderBook.bidsAsksForDepth);
    ...
    yAxis: [
      {
        ...
        axisLabel: {
          ...
          formatter: function (value) {
            return formatBigNumber(value, true, 0);
          }
        },
        splitLine: {
          show: false
  ...
  const draw = (bidAskData, lastPrice, priceTick) => {
    if (!depthChart.current || !bidAskData?.bids?.length || !bidAskData?.asks?.length) return;
  ...
  //     let numBids = Number(new Big(Number(finalBids[1][0]) || 0).minus(min).div(Number(priceTick) || 0).toString());
  //     for (let i = 1; i < numBids; i++) {
  //       let temp = new Big(Number(finalBids[0][0]) || 0).minus(Number(priceTick) || 0).toString();
  ...
  //     let numAsks = Number(
  //       new Big(Number(max) || 0)
  //         .minus(Number(finalAsks[finalAsks.length - 1][0]) || 0)
  //         .div(Number(priceTick) || 0)
  //         .toString()
  //     );
  //     for (let i = 1; i < numAsks; i++) {
  //       let temp = new Big(finalAsks[finalAsks.length - 1][0]).plus(priceTick).toString();
  ...
  useEffect(() => {
    setLastPrice(futureQuot?.lastPrice || 0);
  }, [futureQuot]);
